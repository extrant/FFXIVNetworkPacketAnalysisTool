...
namespace FFXIVNetworkPacketAnalysisTool.Windows
{
    ...
    {
        /// <summary>
        /// 将 Web 坐标系（ushort）转换为游戏内原始坐标（float）
        /// 参考自 Python 代码中的 pos_web_to_raw 函数
        /// </summary>
        /// <param name="webCoord">Web 坐标值 (ushort)</param>
        /// <returns>游戏内原始坐标 (float)</returns>
        public static float PosWebToRaw(ushort webCoord)
        {
            // 将 ushort 坐标转换为实际的游戏坐标
            // FFXIV 中坐标通常是缩放后的值，需要除以适当的缩放因子
            return webCoord / 1000f;
        }

        /// <summary>
        /// 将方向值（ushort）转换为弧度
        /// 参考自 Python 代码中的 dir_web_to_raw 函数
        /// </summary>
        /// <param name="webDir">Web 方向值 (ushort)</param>
        /// <returns>弧度值 (float)</returns>
        public static float DirWebToRaw(ushort webDir)
        {
            // FFXIV 中的方向通常是 0-65535 的无符号整数，表示 0-2π 的弧度
            return (webDir * 2f * (float)Math.PI) / 65536f;
        }

        /// <summary>
        /// 从三个 ushort Web 坐标创建 Vector3
        /// </summary>
        /// <param name="webCoords">ushort 坐标数组 (长度必须为3)</param>
        /// <returns>Vector3 游戏坐标</returns>
        public static Vector3 CreatePosFromWebCoords(ushort[] webCoords)
        {
    ...
    public class MainWindow : Window, IDisposable
    {
        ...
        private unsafe string ReadFieldValue(byte[] data, int offset, Type fieldType, string fieldName, Type? structType = null, List<FieldInfo>? allFields = null)
        {
            ...
            fixed (byte* ptr = data)
            {
                ...
                {
                    if (offset + 11 < data.Length) // Vector3 是 12 字节 (3 * 4)
                    {
                        float x = *(float*)(ptr + offset);
                        float y = *(float*)(ptr + offset + 4);
                        float z = *(float*)(ptr + offset + 8);
                        return $"({x:F2}, {y:F2}, {z:F2})";
                    }
                    return "N/A";
                }

                // 处理 fixed 数组（编译器会生成特殊的嵌套类型）
                if (fieldType.Name.Contains("<") || fieldType.IsNestedPublic || fieldType.Name.Contains("e__FixedBuffer"))
                {
                    ...
                    if (lowerFieldName.Contains("pos") && structType != null)
                    {
                        ...
                        // 需要使用 pos_web_to_raw 转换的结构体
                        bool needsWebToRawConversion = structName == "DOWN_ActorMove" ||
                                                     structName == "DOWN_ActorCast" ||
                                                     structName.Contains("DOWN_AoeEffect");

                        if (needsWebToRawConversion && fieldType.Name.Contains("UInt16"))
                        {
                            // 读取 3 个 ushort 坐标值并转换为 Vector3
                            if (offset + 5 < data.Length) // 3 * sizeof(ushort) = 6 bytes
                            {
                                ...
                                var pos = CoordinateConverter.CreatePosFromWebCoords(webCoords);
                    ...
                    {
                        int maxBytes = Math.Min(32, data.Length - offset);
                        if (maxBytes > 0)
                        {
                            var nameBytes = new byte[maxBytes];
                            Array.Copy(data, offset, nameBytes, 0, maxBytes);
                            var name = System.Text.Encoding.UTF8.GetString(nameBytes).TrimEnd('\0');
                            return string.IsNullOrEmpty(name) ? "[空名称]" : name;
                        }
                        return "[名称] 数据不足";
                    }
                    ...
                    {
                        int remainingBytes = Math.Min(16, data.Length - offset);
                        if (remainingBytes > 0)
                        {
